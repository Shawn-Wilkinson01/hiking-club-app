...
router.get("/announcements", async (_req, res): Promise<void> => {
  const rows = await db.select().from(announcementsTable).orderBy(desc(announcementsTable.pinned), desc(announcementsTable.createdAt));
  res.json(ListAnnouncementsResponse.parse(rows.map((a) => ({ ...a, createdAt: a.createdAt.toISOString() }))));
});
...
router.post("/announcements", async (req, res): Promise<void> => {
  ...
  res.status(201).json(GetAnnouncementResponse.parse({ ...ann, createdAt: ann.createdAt.toISOString() }));
});
