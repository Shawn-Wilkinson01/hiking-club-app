...
router.get("/trails", async (req, res): Promise<void> => {
  ...
  res.json(ListTrailsResponse.parse(rows.map((t) => ({
    ...t,
...
router.post("/trails", async (req, res): Promise<void> => {
  ...
  res.status(201).json(GetTrailResponse.parse({ ...trail, createdAt: trail.createdAt.toISOString() }));
});
...
router.get("/trails/:id", async (req, res): Promise<void> => {
  ...
  const [trail] = await db.select().from(trailsTable).where(eq(trailsTable.id, params.data.id));
  if (!trail) {
...
router.patch("/trails/:id", async (req, res): Promise<void> => {
  ...
  const [trail] = await db.update(trailsTable).set(parsed.data).where(eq(trailsTable.id, params.data.id)).returning();
  if (!trail) {
    res.status(404).json({ error: "Trail not found" });
    return;
  }
...
router.delete("/trails/:id", async (req, res): Promise<void> => {
  ...
  const [trail] = await db.delete(trailsTable).where(eq(trailsTable.id, params.data.id)).returning();
  if (!trail) {
    res.status(404).json({ error: "Trail not found" });
    return;
  }
