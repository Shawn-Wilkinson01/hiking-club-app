...
router.get("/members", async (_req, res): Promise<void> => {
  ...
  res.json(ListMembersResponse.parse(rows.map((m) => ({ ...m, joinedAt: m.joinedAt.toISOString() }))));
});
...
router.post("/members", async (req, res): Promise<void> => {
  ...
  res.status(201).json(GetMemberResponse.parse({ ...member, joinedAt: member.joinedAt.toISOString() }));
});
